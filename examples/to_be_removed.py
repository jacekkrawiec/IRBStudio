...
target_auc=0.85
application_start_date = pd.to_datetime('2024-12-01')
...
df = simulated_portfolio_df.loc[simulated_portfolio_df['observation_date'] < pd.to_datetime('2024-12-01')]
df = df.loc[df['default_flag'] == 0]


roc_auc_score(df['into_default_flag'], df['simulated_score'])

sns.histplot(df, x='simulated_score', hue='into_default_flag', stat='density', common_norm=False, bins=30)
plt.show()
